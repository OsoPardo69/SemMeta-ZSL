#!/usr/bin/env python3
"""
run_hgbt_combined_multiseed.py
═══════════════════════════════════════════════════════════════════════════════
LightGBM Baseline Combiner for VENOM (CIC-AndMal-2020)
(Drop-in GPU replacement for sklearn's HistGradientBoostingClassifier)
...
CFG = {
    "lgbm_params": dict(
        n_estimators=500,  # Equivalent to max_iter in sklearn's HGB
        learning_rate=0.05,
        max_depth=6,
        class_weight="balanced",
        verbose=-1,  # Quiet mode
        device="gpu" if torch.cuda.is_available() else "cpu",  # Hardware acceleration!
    ),
}
...
def print_summary_table(gzsl_results, ceiling_results, n_seeds):
    print(f"\n{BANNER}")
    print(f" HgBT / LightGBM ONE-SHOT GZSL RESULTS   (mean ± std, %,  {n_seeds} seeds)")
    print(f"{BANNER}")
...
def main():
    print(f"\n{BANNER}")
    print("  VENOM — HistGradientBoosting (LightGBM) Multi-Seed Pipeline")
    ...
    for ds_name, ds_path in DATASETS.items():
        ...
        for idx, seed in enumerate(SEEDS):
            ...
            try:
                clf_gzsl = lgb.LGBMClassifier(random_state=seed, **CFG["lgbm_params"])
                clf_gzsl.fit(X_train_gzsl, y_train_gzsl)
            except Exception as e:
                # Fallback to CPU if LightGBM wasn't compiled with GPU support locally
                fallback_params = CFG["lgbm_params"].copy()
                if "device" in fallback_params: del fallback_params["device"]
                clf_gzsl = lgb.LGBMClassifier(random_state=seed, **fallback_params)
                clf_gzsl.fit(X_train_gzsl, y_train_gzsl)

            # Extract probabilities and convert to pseudo-logits
            ...
            try:
                clf_ceil = lgb.LGBMClassifier(random_state=seed, **CFG["lgbm_params"])
                clf_ceil.fit(X_train_ceil, y_train_ceil)
            except Exception as e:
                fallback_params = CFG["lgbm_params"].copy()
                if "device" in fallback_params: del fallback_params["device"]
                clf_ceil = lgb.LGBMClassifier(random_state=seed, **fallback_params)
                clf_ceil.fit(X_train_ceil, y_train_ceil)
